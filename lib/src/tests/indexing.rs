...
pub fn should_index_existing_vertex_property<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    // Get the vertex
    let result = util::get_vertices(db, models::VertexWithPropertyPresenceQuery::new(property_name.clone()))?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);

    // Get the vertex with piped queries
    let result = util::get_vertices(db, q.clone().with_property(property_name.clone())?)?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    let result = util::get_vertices(db, q.clone().without_property(property_name.clone())?)?;
    assert!(result.is_empty());

    // Check against another property
    let result = util::get_vertices(db, q.clone().without_property(other_property_name.clone())?);
    expect_err!(result, Error::NotIndexed);
    db.index_property(&other_property_name.clone())?;
    let result = util::get_vertices(db, q.without_property(other_property_name.clone())?)?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    Ok(())
}
...
pub fn should_index_existing_edge_property<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    // Get the edge
    let result = util::get_edges(db, models::EdgeWithPropertyPresenceQuery::new(property_name.clone()))?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0], edge);

    // Get the edge with a piped query
    let result = util::get_edges(db, q.clone().with_property(property_name.clone())?)?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0], edge);
    let result = util::get_edges(db, q.clone().without_property(property_name.clone())?)?;
    assert!(result.is_empty());

    // Check against another property
    let result = util::get_edges(db, q.clone().without_property(other_property_name.clone())?);
    expect_err!(result, Error::NotIndexed);
...
pub fn should_update_indexed_vertex_property<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    assert_eq!(result.len(), 0);
    let result = util::get_vertices(db, q.clone().with_property_equal_to(property_name.clone(), json_true.clone())?)?;
    assert_eq!(result.len(), 0);
    let result = util::get_vertices(
        db,
        q.clone().with_property_not_equal_to(property_name.clone(), json_true.clone())?,
    )?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    ...
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    let result = util::get_vertices(db, q.clone().with_property_equal_to(property_name.clone(), json_false.clone())?)?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    let result = util::get_vertices(db, q.with_property_not_equal_to(property_name.clone(), json_false.clone())?)?;
    assert_eq!(result.len(), 0);
...
pub fn should_update_indexed_edge_property<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    assert_eq!(result.len(), 0);
    let result = util::get_edges(db, q.clone().with_property_equal_to(property_name.clone(), json_true.clone())?)?;
    assert_eq!(result.len(), 0);
    let result = util::get_edges(db, q.clone().with_property_not_equal_to(property_name.clone(), json_true)?)?;
    assert_eq!(result.len(), 1);
    ...
    assert_eq!(result.len(), 1);
    assert_eq!(result[0], edge);
    let result = util::get_edges(db, q.clone().with_property_equal_to(property_name.clone(), json_false.clone())?)?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0], edge);
