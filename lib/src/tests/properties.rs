...
pub fn should_handle_vertex_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let t = Identifier::new("https://example.org/test_vertex_type")?;
    let id = db.create_vertex_from_type(t)?;
    let q = SpecificVertexQuery::single(id);

    // Check to make sure there's no initial value
    let result = util::get_vertex_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/foo")?))?;
    assert_eq!(result.len(), 0);

    // Set and get the value as true
    db.set_properties(q.clone(), &Identifier::new("https://example.org/foo")?, &ijson!(true))?;
    let result = util::get_vertex_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/foo")?))?;
    ...
    db.set_properties(q.clone(), &Identifier::new("https://example.org/foo")?, &ijson!(false))?;
    let result = util::get_vertex_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/foo")?))?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, id);
    ...
    db.delete(q.clone().properties()?.name(Identifier::new("https://example.org/foo")?))?;
    let result = util::get_vertex_properties(db, q.properties()?.name(Identifier::new("https://example.org/foo")?))?;
    assert_eq!(result.len(), 0);
...
pub fn should_get_all_vertex_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let t = Identifier::new("https://example.org/a_vertex")?;
    ...
    db.set_properties(q2.clone(), &Identifier::new("https://example.org/a")?, &ijson!(false))?;
    db.set_properties(q2.clone(), &Identifier::new("https://example.org/b")?, &ijson!(true))?;
    ...
    assert_eq!(result_2[0].props[0].name, Identifier::new("https://example.org/a")?);
    assert_eq!(result_2[0].props[0].value, ijson!(false));
    assert_eq!(result_2[0].props[1].name, Identifier::new("https://example.org/b")?);
...
pub fn should_not_set_invalid_vertex_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    db.set_properties(q.clone(), &Identifier::new("https://example.org/foo")?, &ijson!(null))?;
    let result = util::get_vertex_properties(db, q.properties()?.name(Identifier::new("https://example.org/foo")?))?;
    assert_eq!(result.len(), 0);
    Ok(())
}
...
pub fn should_not_delete_invalid_vertex_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let q = SpecificVertexQuery::single(Uuid::default())
        .properties()?
        .name(Identifier::new("https://example.org/foo")?);
    db.delete(q)?;
...
pub fn should_handle_edge_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let vertex_t = Identifier::new("https://example.org/test_vertex_type")?;
    let outbound_id = db.create_vertex_from_type(vertex_t.clone())?;
    let inbound_id = db.create_vertex_from_type(vertex_t.clone())?;
    let edge_t = Identifier::new("https://example.org/test_edge_type")?;
    ...
    // Check to make sure there's no initial value
    let result = util::get_edge_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/edge-property")?))?;
    assert_eq!(result.len(), 0);

    // Set and get the value as true
    db.set_properties(q.clone(), &Identifier::new("https://example.org/edge-property")?, &ijson!(true))?;
    let result = util::get_edge_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/edge-property")?))?;
    assert_eq!(result.len(), 1);
    ...
    // Set and get the value as false
    db.set_properties(q.clone(), &Identifier::new("https://example.org/edge-property")?, &ijson!(false))?;
    let result = util::get_edge_properties(db, q.clone().properties()?.name(Identifier::new("https://example.org/edge-property")?))?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].edge, edge);
    assert_eq!(result[0].value, ijson!(false));
    ...
    // Delete & check that it's deleted
    db.delete(q.clone())?;
    let result = util::get_edge_properties(db, q.properties()?.name(Identifier::new("https://example.org/edge-property")?))?;
    assert_eq!(result.len(), 0);
...
pub fn should_get_all_edge_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    // Set and get the value as true
    db.set_properties(eq.clone(), &Identifier::new("https://example.org/edge-prop-1")?, &ijson!(false))?;
    db.set_properties(eq.clone(), &Identifier::new("https://example.org/edge-prop-2")?, &ijson!(true))?;

    let result = util::get_all_edge_properties(db, eq.clone())?;
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].props.len(), 2);
    assert_eq!(result[0].props[0].name, Identifier::new("https://example.org/edge-prop-1")?);
    assert_eq!(result[0].props[0].value, ijson!(false));
    assert_eq!(result[0].props[1].name, Identifier::new("https://example.org/edge-prop-2")?);
    assert_eq!(result[0].props[1].value, ijson!(true));

    // Delete & check that they are deleted
    db.delete(eq.clone().properties()?.name(Identifier::new("https://example.org/edge-prop-1")?))?;
    db.delete(eq.clone().properties()?.name(Identifier::new("https://example.org/edge-prop-2")?))?;
...
pub fn should_not_set_invalid_edge_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let edge = Edge::new(Uuid::default(), Identifier::new("https://example.org/foo")?, Uuid::default());
    let q = SpecificEdgeQuery::single(edge);
    db.set_properties(q.clone(), &Identifier::new("https://example.org/bar")?, &ijson!(null))?;
    let result = util::get_edge_properties(db, q.properties()?.name(Identifier::new("https://example.org/bar")?))?;
    assert_eq!(result.len(), 0);
    Ok(())
}
...
pub fn should_not_delete_invalid_edge_properties<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    db.delete(
        SpecificEdgeQuery::single(edge)
            .properties()?
            .name(Identifier::new("https://example.org/bar")?),
    )?;

    let outbound_id = db.create_vertex_from_type(Identifier::new("https://example.org/foo")?)?;
    let inbound_id = db.create_vertex_from_type(Identifier::new("https://example.org/foo")?)?;

    let edge = Edge::new(outbound_id, Identifier::new("https://example.org/baz")?, inbound_id);
    db.create_edge(&edge)?;
    db.delete(
        SpecificEdgeQuery::single(edge)
            .properties()?
            .name(Identifier::new("https://example.org/bleh")?),
    )?;
...
pub fn should_get_a_vertex_properties_count<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    ...
    let q = SpecificVertexQuery::single(id);
    db.set_properties(q.clone(), &Identifier::new("https://example.org/foo")?, &ijson!(true))?;
    let count = extract_count(db.get(q.properties()?.name(Identifier::new("https://example.org/foo")?).count()?)?).unwrap();
    assert!(count >= 1);
    Ok(())
}

pub fn should_not_set_properties_on_count<D: Datastore>(db: &Database<D>) -> Result<(), Error> {
    let result = db.set_properties(AllVertexQuery.count()?, &Identifier::new("https://example.org/foo")?, &ijson!(true));
