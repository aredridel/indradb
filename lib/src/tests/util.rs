...
pub(crate) fn create_edge_from<D: Datastore>(db: &Database<D>, outbound_id: Uuid) -> Result<Uuid> {
    let inbound_vertex_t = models::Identifier::new("https://example.org/test_inbound_vertex_type")?;
...
pub(crate) fn create_edges<D: Datastore>(db: &Database<D>) -> Result<(Uuid, [Uuid; 5])> {
    let outbound_vertex_t = models::Identifier::new("https://example.org/test_outbound_vertex_type")?;
